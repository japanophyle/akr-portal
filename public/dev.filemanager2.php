<?php
session_start();
$user=(isset($_SESSION['userArray']))?$_SESSION['userArray']:false;
$fm_session=(isset($_SESSION['filemanager']))?$_SESSION['filemanager']:false;
$show_login=false;
$zip_overwrite=false;
$base_url='/Member.me/';
if($user){
	if($user['access']>=30){
		if(!$fm_session){
			$fm_session['logged']='admin';
			$_SESSION['filemanager']=$fm_session;
		}
	}else{
...
function renderMenuItems(){
	global $base_url;
	$items=[
		'admin'=>['title'=>'Admin Dashboard','href'=>$base_url.'admin/','label'=>'<i class="fa fa-dashboard" aria-hidden="true"></i> Admin Home'],
		//'zipper'=>['title'=>'Dev Zipper','href'=>$base_url.'/zipper','label'=>'<i class="fa fa-archive" aria-hidden="true"></i> Zipper'],
		//'db'=>['title'=>'DB Manager','href'=>'/tbs_desk3/public/tinyDBmanager.php','label'=>'<i class="fa fa-database" aria-hidden="true"></i> Database'],
		//'log'=>['title'=>'Log Viewer','href'=>'/tbs_desk3/public/tinylogviewer.php','label'=>'<i class="fa fa-bug" aria-hidden="true"></i> Error Log'],
	];
	$out='';
	foreach($items as $v){
		$out.='<a title="'.$v['title'].'" class="dropdown-item nav-link" href="'.$v['href'].'">'.$v['label'].'</a>';
	}
	return $out;
}
//settings for ace
$ace_edit_types="{'php':'php','html':'html','txt':'text','js':'javascript'}";
$menu_items=renderMenuItems();

$custom_css='#main-table_wrapper{height:82vh;margin-bottom:1rem;overflow-y:auto; border-bottom:1px solid #ccc;}
	#main-table thead th{position:sticky;top:0; background-color:#ccc;};
	/* colors */
	/* links */
	.nav-link{color:#3600FF;}
	/* icons */
	.inline-actions > a > i{background-color:#FF9A00; color:#333;}';
$custom_css_dark='#main-table thead th{background-color:rgba(255,255,255,1);color:#333;}';

//Default Configuration
$CONFIG = '{"lang":"en","lock_path":"","error_reporting":false,"show_hidden":false,"hide_Cols":false,"theme":"light","zip_overwrite":false}';
...
$auth_users = array(
    'adminx' => '$2y$10$/K.hjNr84lLNDt8fTXjoI.DBp6PpeyoJ.mGwrrLuCZfAwfSAGqhOW', //admin@123
    'admin' => '$2y$10$H0CQvWzI.DRPBO4MIekYgOH8Q04l6czNnFUyWi7dUgjVXgEnXaE7S', //jamdaze!
...
// Theme
$theme = 'dark';//isset($cfg->data['theme']) ? $cfg->data['theme'] : 'light';
                        ...
                        <div class="card fat <?php echo fm_get_theme(); ?>">
                            <div class="card-body">
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    ...
                                    <?php if(!$show_login){ ?>
										<div class="mb-3">
											<p class="text-center">Sorry, you need to login via the <a title="Joho Dashboard" href="<?php echo $base_url; ?>">main app</a>.</p>
										</div>										
									<?php }else{ ?>
                                    <div class="mb-3">
...
if (isset($_POST['ajax'], $_POST['token']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "settings") {
        global $cfg, $lang, $report_errors, $show_hidden_files, $lang_list, $hide_Cols, $theme,$zip_overwrite;
        ...
        $zow = isset($_POST['js-zip-overwrite']) && $_POST['js-zip-overwrite'] == "true" ? true : false;
        $te3 = $_POST['js-theme-3'];
$cfg->data['lock_path']=$_POST['lock_path'];
        ...
        if ($cfg->data['zip_overwrite'] != $zow) {
            $cfg->data['zip_overwrite'] = $zow;
            $zip_overwrite = $zow;
        }
        $cfg->save();
        echo true;
            ...
            <div class="card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
					...
					<div class="mt-3 mb-3 row ">
						<label for="lock_path" class="col-sm-3 col-form-label">Restrict FM Directory</label>
						<div class="col-sm-9">
							<input type="text" class="form-control" id="lock_path" name="lock_path" value="<?php echo $cfg->data['lock_path'];?>"/>
						 </div>
					</div>
                    </div>
                   <div class="mt-3 mb-3 row ">
 ...
 class FM_Config
{
    ...
    {
        global $root_path, $root_url,$lock_path, $CONFIG;
        ...
        $this->valid_path=$root_path.$lock_path;
        $this->valid_url=$root_url.$lock_path;
        $data = false;
        if (strlen($CONFIG)) {
        ...
        if (is_array($data) && count($data)){
			$this->data = $data;
			$this->valid_path=$root_path.$data['lock_path'];
			$this->valid_url=$root_url.$data['lock_path'];
 		}
        else $this->save();
    }
...
{
    global $lang, $sticky_navbar, $editFile,$menu_items;
...
global $lang, $root_url, $sticky_navbar, $favicon_path,$custom_css,$custom_css_dark;
...
<style>
    ...
    if (FM_THEME == "dark"): ?>
        <style>
            ...
            body.theme-dark { background-color:#101010; color:#CFD8DC; }
            .list-group .list-group-item { background: #343a40; }
            .theme-dark .navbar-nav i, .navbar-nav .dropdown-toggle, .break-word { color: #CFD8DC; }
            a, a:hover, a:visited, a:active, #main-table .filename a, i.fa.fa-folder-o, i.go-back { color: #85bafd; }
            ul#search-wrapper li:nth-child(odd) { background: #f9f9f9cc; }
            .theme-dark .btn-outline-primary { color: #85bafd; border-color: #85bafd; }
            ...
            .theme-dark .inline-actions > a > i { background: #85bafd; }
