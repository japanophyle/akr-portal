<?php
/**
 * All of the Defines for the simple_html_dom classes.
...
class slimHtmlDom{
	...
	//locate images
	public function findImages($html=false,$return=false){
		$src=false;
		if($html){
			$DM=$this->str_get_html($html);
			foreach($DM->find('img') as $element){
				$src[]=$element->src;
			}
		}
		return $src;
