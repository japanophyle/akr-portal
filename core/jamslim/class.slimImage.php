...
class slimImage {
	...
	public function get($function=false,$args=false){
	...
	private function init(){
		...
		$this->DEFAULT_IMAGE='gfx/MerlinLogo_badge.png';
	}
	...
	private function getImageSRC($args) {
		...
		$noimage=issetCheck($args,'default');
		if(!$noimage) $noimage = $this->DEFAULT_IMAGE;
		if (!isset($src)||$src=='') {
			$out['id'] = 0;
		...
		//check exists
		if (!file_exists($out['image'])) $out['image'] = $noimage;
		if (!file_exists($out['thumb'])) $out['thumb'] = $noimage;
		return $out;
	}
	...
	private function getImageCredit($args) {
		...
		$info = (issetOR($meta['info'])) ? '<dd class="info">' . fixHTML($meta['info']) . '</dd>' : false;
		$own = ($info) ? '<a href="#nogo" class="jTip" title="more info">&copy; ' . fixHTML($meta['credit']) . '</a>' : '&copy; ' . fixHTML($meta['credit']);
		$owner = (issetOR($meta['credit'])) ? '<dd class="owner">' . $own . '</dd>' : false;
	...
	private function getImageGallery($args) {
		$out='';
		$data=issetCheck($args,'data');
		$size=issetCheck($args,'size');
		$class=issetCheck($args,'class');
		if (!$data||$data==='NULL') return false;
		$chk = unserialize($data);
		$ids = (is_array($chk)) ? implode(',', $chk) : $data;
		$order = (is_array($chk)) ? $chk : explode(',', $ids);
		...
		$recs =(issetOR($ids))?runQuery($sql, 'obj'):false;
	...
	private function getAvatar($args) {
		$code=issetCheck($args,'code');
		$img=issetCheck($args,'img');
		$hash=issetCheck($args,'hash');
		$size=issetCheck($args,'size',150);
	...
	private function getHashFromAvatar($args=false){
		$path=issetCheck($args,'path');
		if(!$path) $path=issetCheck($args,'data');
