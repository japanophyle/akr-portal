...
{SHEETS}
	...
	public static function fromArray( array $rows, $sheetName = null ,$addHeader=false) {
		$xlsx = new static();
		return $xlsx->addSheet( $rows, $sheetName,$addHeader );
	}

	public function addSheet( array $rows, $name = null, $addHeader = false ) {
		$this->curSheet++;
		...
		if(is_array($rows)){
			if($addHeader){
				$tmp['headers']=array_keys(current($rows));
				foreach($rows as $i=>$r) $tmp[$i]=$r;
				$rows=$tmp;
			}
